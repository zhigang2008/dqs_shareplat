...
function batchDelete(action,checkboxName,form){
    if (!hasOneChecked(checkboxName)){
            showAlert("提示","请选择要操作的对象!",200,100);
            return;
    }
    
    showConfirm("确定","确定执行[删除]操作?",
    		function(confirmval){
    	      if(confirmval){
    	    	  form.action = action;
                  form.submit();
...
Popup.prototype = {
    ...
    build: function () {
        ...
        var close = '<div id="dialogBoxClose"  ></div>';
    ...
    lastBuild: function () {
        ...
        if (this.config.contentType == 1) {
            ...
            Event.observe(G('dialogOk'), "click", this.forCallback_OK.bindAsEventListener(this), false);
            Event.observe(G('dialogCancel'), "click", this.forCallback_Cancel.bindAsEventListener(this), false)
    ...
    forCallback_OK: function () {
    	  this.close();
        this.info.callBack(true);
      },
    forCallback_Cancle: function () {
    	  this.close();
        this.info.callBack(false);
      },
    shadow: function () {
...
Popup.prototype = {
    ...
    lastBuild: function () {
        ...
        if (this.config.contentType == 1) {
            ...
            G("dialogBody").innerHTML = confirm;
            Event.observe(G('dialogOk'), "click", this.forCallback_OK.bindAsEventListener(this), false);
            Event.observe(G('dialogCancel'), "click", this.forCallback_Cancle.bindAsEventListener(this), false);
        } else if (this.config.contentType == 4) {
            G("dialogBody").innerHTML = alert;
            Event.observe(G('dialogYES'), "click", this.close.bindAsEventListener(this), false);
        } else if (this.config.contentType == 5) {
            G("dialogBody").innerHTML = alert;
            Event.observe(G('dialogYES'), "click", this.forCallback_Response.bindAsEventListener(this), false);
        }
    },
    reBuild: function () {
        G('dialogBody').height = G('dialogBody').clientHeight;
...
//update by Steven 2010-3-6 
      var _inner_pop=null;
           
       function showFrame(title,contentUrl,width,height)
        {
           _inner_pop=new Popup({ contentType:1,isReloadOnClose:false,width:width,height:height});
           _inner_pop.setContent("contentUrl",contentUrl);
           _inner_pop.setContent("title",title);
           _inner_pop.build();
           _inner_pop.show();
        }
        function showHtmlString(title,strHtml,width,height)
        {
            _inner_pop=new Popup({ contentType:2,isReloadOnClose:false,width:width,height:height});
            _inner_pop.setContent("contentHtml",strHtml);
            _inner_pop.setContent("title",title);
            _inner_pop.build();
            _inner_pop.show();
       }
        function showConfirm(title,confirmCon,callbackfunction,width,height)
        {
        	 
            var _inner_pop=new Popup({ contentType:3,isReloadOnClose:false,width:width,height:height});
            _inner_pop.setContent("title",title);
            _inner_pop.setContent("confirmCon",confirmCon);
            _inner_pop.setContent("callBack",callbackfunction);
            _inner_pop.setContent("someHiddenEle","dialogBoxClose");
            _inner_pop.build();
            _inner_pop.show();
        ...
        {
            _inner_pop=new Popup({ contentType:4,isReloadOnClose:false,width:width,height:height});
            _inner_pop.setContent("title",title);
            _inner_pop.setContent("alertCon",alertCon);
            _inner_pop.build();
            _inner_pop.show();
        }
        function showResponseAlert(title,alertCon,callbackfunction,width,height)
        {
            _inner_pop=new Popup({ contentType:5,isReloadOnClose:false,width:width,height:height});
            _inner_pop.setContent("title",title);
            _inner_pop.setContent("alertCon",alertCon);
            _inner_pop.setContent("callBack",callbackfunction);
            _inner_pop.setContent("someHiddenEle","dialogBoxClose");
...
/**内嵌提示框 **/
        var s_j=100;
        function change_hidden(target){
        	var obj=target;
        	s_j=s_j-1;
        	obj.style.filter="alpha(opacity="+s_j+")";
        	obj.style.opacity=j/100;
        	if(s_j<=0){
        		clearInterval(h);obj.style.display="none";s_j=100;
        		}
        	}
        function msgTip(msg,msgtype,target){
        	var obj=$(target);
        	obj.innerHTML=msg;
        	if(typeof(msgtype)=="undefined"||msgtype==null||msgtype=="info"){
        		obj.className="msginfo";
        		h=setInterval(change_hidden(obj),2);
        		}
           else if(msgtype=="error"){
        	   obj.className="msgerrors";
        	   }
           else if(msgtype=="success"){
        	   obj.className="msgsuccess";
        	   h=setInterval(change_hidden(obj),2);
        	   }
           else{
        	   obj.className="msginfo";
